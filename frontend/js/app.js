...
// Reload page when navigating back (from browser cache)
...
function showApp() {
    ...
    // Initialize app with error handling to prevent crashes
    initApp().catch(err => {
        console.error("Error initializing app:", err);
        // Don't show error toast - app might still be partially functional
    });
}
...
async function handleSignUp(e) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (res.status === 409) {
                console.error("Sign-up conflict: Email already exists");
                showAuthError("This email is already registered. Please sign in or use a different email.");
            } else {
...
async function handleSignIn(e) {
    ...
    // Prevent duplicate submissions
    if (signinInProgress) {
        console.log("Sign-in already in progress, ignoring duplicate submission");
        return;
    }
